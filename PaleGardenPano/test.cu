...
#pragma unroll
    for (int i = 0; i < 6; i += 2)
    {
        ...
        if (patchGenerates(xrand)) return true; // precondition not satisfied
    }
...
__device__ void testWorldseed(uint64_t worldseed)
{
    ...
    // in chunk (217, -103): flower patch
    uint64_t popseed = xGetPopulationSeed(&xrand, worldseed, 217 << 4, -103 << 4);
    ...
    // if no flower patch in chunks (216, -100), (216, -99), (215, -99) then 
    // in chunk(215, -100) : flower patch
    const int chunks1[] = { 216, -100,   216, -99,   215, -99,   215, -100 };
    if (!conditionalCheck(&xrand, worldseed, chunks1)) return;

    // if no flower patch in chunk (217, -98) then 
    // in chunk (217,-99) : flower patch
    const int chunks2[] = { 217, -98,   217, -98,   217, -98,   217, -99 };
    ...
    // if no flower patch in chunks (218, -104) then
    // in chunk (217, -104) : flower patch (or unlikely single flower at 15, 7)
    const int chunks3[] = { 218, -104,   218, -104,   218, -104,   217, -104 };
    if (!conditionalCheck(&xrand, worldseed, chunks3))
    {
        // try single flower (pretty hopeless but whatever)
        xSetDecoratorSeed(&xrand, worldseed, 217 << 4, -104 << 4, SINGLE_FLOWER_SALT);
        if (!singleFlowerGenerates(&xrand, 15, 7))
            return;
    }
...
int main()
{
    ...
    printf("Got %llu candidates total\n", seedCounter);
