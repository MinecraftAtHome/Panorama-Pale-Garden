...
#define TEST_FILE_FLOWERS "data/fulltest_flowers.txt"
#define TEST_FILE_AIRBLOCKS "data/fulltest_airblocks.txt"


constexpr uint64_t CORRECT_SEED = 1905538119ULL;
...
__device__ void initialFilter(const uint64_t worldseed)
{
    ...
    if (!testFlowerInChunkUnconditional(&xrand, worldseed, { 75, -93 }, { 6, 5 }))
        return;

    // conditional filters

    const ChunkPos chunks1[] = QUAD_CHUNK(72, -90, 1, -1);
    const BlockPos2D flower1 = { 11, 3 };
    if (!testFlowerInChunkConditional(&xrand, worldseed, chunks1, flower1))
        return;

    const ChunkPos chunks2[] = QUAD_CHUNK(74, -95, 1, -1);
    const BlockPos2D flower2 = { 13, 5 };
    if (!testFlowerInChunkConditional(&xrand, worldseed, chunks2, flower2))
        return;

    const ChunkPos chunks3[] = QUAD_CHUNK(78, -90, 1, 1);
    const BlockPos2D flower3 = { 13, 15 };
	...
	if (worldseed == CORRECT_SEED)
		printf("Correct seed found in filter 1\n");
...
__device__ void secondaryFilter(const uint64_t worldseed)
{
    ...
    if (usedFlowers == fullMask)
    {
        ...
        if (worldseed == CORRECT_SEED)
            printf("Correct seed found in filter 2\n");
    }
}
...
__device__ void finalFilter(uint64_t worldseed)
{
    ...
    if (worldseed == CORRECT_SEED)
        printf("Correct seed found in filter 3\n");
}
...
#pragma unroll
    for (uint32_t low = 0; low < 4; low++)
    {
        ...
        const uint64_t worldseed = (firstState >> 16) + ((int)(secondState >> 16));
...
int runCrackerRandomSeeds()
{
    ...
    for (int run = 0; run < NUM_RUNS_RANDOM_SEEDS; run++)
    {
        if (run % 100 == 0)
        ...
        crackRandomSeedPart1 << < NUM_BLOCKS_1, THREADS_PER_BLOCK >> > (run * RANDOM_SEEDS_PER_RUN);
        CHECKED_OPERATION(cudaGetLastError());
        CHECKED_OPERATION(cudaDeviceSynchronize());
        //printf("After filter 1: %d\n", resultID1);
