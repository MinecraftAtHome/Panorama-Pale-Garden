...
__host__ __device__ inline uint64_t getPopulationSeedFast(const SeedConstants& sc, const int chunkX, const int chunkZ)
{
    const uint64_t popseed = sc.A * chunkX + sc.B * chunkZ ^ sc.worldseed;
    return popseed;
}

__host__ __device__ inline uint64_t getPopulationSeedFast(const SeedConstants& sc, const ChunkPos& chunkPos)
{
