...
__device__ void secondaryFilter(const uint64_t worldseed)
{
    ...
    const uint64_t fullMask = (1ULL << targetFlowerCount) - 1;
        ...
        for (int dcz = -1; dcz <= 1; dcz++)
        {
            ...
            for (int y = 0; y < 7; y++)
            {
                ...
                for (int i = 0; i < targetFlowerCount; i++)
                {
                    ...
                    {
                        if ((matchedFlowerMask & (1ULL << i)) != 0)
                            continue;
                        matchedFlowerMask |= 1ULL << i;
                        matchedFlowerCount++;
                    }
                }
            }
